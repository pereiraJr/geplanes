...
<n:property name="${name}">
	...
	<script type="text/javascript">
		...
		if (typeof(_popupUG) == 'undefined') {window._popupUG = {}};
		...
		$(form['planoGestao']).focus(function(){
			this.ultimoValor = $(this).val();
		});
		
		$(form['planoGestao']).blur(function(){
			if ($(this).val() != this.ultimoValor){
				$("#${name}_label").unautocomplete();
				...
				if(form['planoGestao'].value != "<null>"){
					...
					if (typeof(_popupUG) != 'undefined'){
						if (typeof(_popupUG.document) != 'undefined'){
							try {
								var propriedade = _popupUG.document.propriedade;
								_popupUG.document.location = '${pageContext.request.contextPath}/util/UnidadeGerencialTreeView?propriedade='+propriedade+'&unidadeGerencial='+form['${name}'].value+'&planoGestao='+form['planoGestao'].value;
							} catch (err) {};
						}
					}
		...
		function formatResult(row) {
	 	...
	 	function escolherUnidadeGerencial${name}(){
	 		if(document.getElementById('botaoEscolher${name}').innerHTML != 'Limpar'){
	 			if (form['planoGestao'].value != '<null>') {
		 			_popupUG = window.open('${pageContext.request.contextPath}/util/UnidadeGerencialTreeView?propriedade=${name}&unidadeGerencial='+form['${name}'].value+'&planoGestao='+form['planoGestao'].value, 'ugtv', 'directories=no,height=450,width=500,top=50,left=200,location=no,menubar=no,status=no,toolbar=no,resizable=yes');
		 		}
		 		else {
